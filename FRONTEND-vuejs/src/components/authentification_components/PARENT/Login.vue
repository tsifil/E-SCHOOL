          ...
          <div class="row w-100 mx-0">
            <div class="col-lg-4 mx-auto">
              <div class="auth-form-light text-left py-5 px-4 px-sm-5">
                ...
                <h6 class="font-weight-light" id="t">Connectez-vous pour continuer.</h6>
                <form class="pt-3">
                  ...
                  <div class="mt-3">
                    <vs-button @click="loginParent()" color="primary" type="gradient" class="btn btn-block btn-success btn-lg font-weight-medium auth-form-btn">SE CONNECTER</vs-button>
                  </div>
  
                  <br><br>
                  <div class="text-center mt-4 font-weight-light">
                    Vous n’avez pas de compte ?  <router-link class="text-primary" :to="{name:'ParentCreatAccount'}">Créer</router-link>
                  </div>
...
async created(){
    if(!this.$store.getters.isLoggedInParent || !this.$store.getters.isAnneeScolaire){
            ...
            this.$router.push('/dashboard_parent')
    }
},
methods:{
  toggleShowPassword() {
  ...
  async loginParent(){
      ...
      if(this.$v.$invalid){
        ...
        try{
              ...
              const response = await AuthService.loginParent(credentials);
              this.msg = response.msg
              const tokenParent = response.tokenParent
              const parent = response.parent
              this.$store.dispatch('loginParent',{tokenParent,parent});
              this.$router.push({name:"anneeScolaireParent"})   
